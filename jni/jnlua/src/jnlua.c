...
static int openlib_protected (lua_State *L) {
	...
	case 9:
		...
		break;
	case 10:
		libname = LUA_MIMELIBNAME;
		openfunc = luaopen_mime_core;
		break;
	case 11:
		libname = LUA_SOCKETLIBNAME;
		openfunc = luaopen_socket_core;
		break;
	case 12:
		libname = LUA_LPEGLIBNAME;
		openfunc = luaopen_lpeg;
		break;
	case 13:
		libname = LUA_SPROTOLIBNAME;
		openfunc = luaopen_sproto_core;
		break;
	case 14:
			libname = LUA_CRYPTLIBNAME;
			openfunc = luaopen_crypt;
			break;
	default:
		return 0;
	}
...
JNIEXPORT void JNICALL Java_com_naef_jnlua_LuaState_lua_1openlib (JNIEnv *env, jobject obj, jint lib) {
	...
	L = getluathread(obj);
	if (checkstack(L, JNLUA_MINSTACK)
			&& checkarg(lib >= 0 && lib <= 14, "illegal library")) {
		openlib_lib = lib;

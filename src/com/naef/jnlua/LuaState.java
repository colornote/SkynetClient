...
public class LuaState {
	...
	private LuaRuntimeException getArgTypeException(int index, LuaType type) {
		return getArgException(index, String.format("%s expected, got %s", type
				.toString().toLowerCase(Locale.US), type(index).toString()
				.toLowerCase()));
	}
	...
	private LuaRuntimeException getArgException(int index, String extraMsg) {
		...
		// Format message
		String msg;
		String argument = index > 0 ? String.format(Locale.US, "argument #%d",
				index) : "self argument";
		if (name != null) {
			msg = String
